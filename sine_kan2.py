...
class SineKANLayer(torch.nn.Module):
    def __init__(self, input_dim, output_dim, device='cuda', grid_size=5, is_first=False, add_bias=True, phase_scaling=0.5):
        ...
        self.phase_scaling = phase_scaling
    ...
    def forward(self, x):
        ...
        s = torch.sin(x_reshaped * self.freq + self.phase_scaling*self.phase)
...
class SineKAN(torch.nn.Module):
    def __init__(
        ...
        self.layers = torch.nn.ModuleList([
            SineKANLayer(
                in_dim, out_dim, grid_size=grid_size, phase_scaling=phase_factor , is_first=True
            ) if i == 0 else SineKANLayer(
                in_dim, out_dim, device, grid_size=grid_size, phase_scaling=phase_factor
